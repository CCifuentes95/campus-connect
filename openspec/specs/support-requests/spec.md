...
## Purpose
The student support-request capability: how a signed-in student creates a support ticket, how that ticket is validated, coded, and audited on creation, and how the student lists, filters, and sorts their own requests. Reads and writes go through `FirebaseServerApp` under the student's own credentials so `firestore.rules` apply; the Admin SDK is never used.
