...
`firestore.rules` SHALL allow a signed-in user to create a notification only in their own
subcollection, only with a `type` in the closed enum above, only with `read == false`, and
only when the document has exactly the required fields.

#### Scenario: A student cannot create a notification for another user

- **WHEN** a signed-in student attempts to write a document to another user's
