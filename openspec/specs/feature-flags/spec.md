...
## Purpose
A lightweight, server-only config feature-flag layer for turning whole flows on/off without an external SaaS (see docs/adr/0006-config-feature-flags.md). A typed registry (`lib/flags.ts`) maps each flag to a non-`NEXT_PUBLIC_*` env var, so no flag value is read on — or inlined into — the client bundle. `isEnabled(name)` is server-only and **safe-default-ON** (off only for a recognised falsey value; unset/malformed = on). Flagged flows gate at both the route entry (render a disabled state) and inside their server action(s) (no write) — defence-in-depth. Toggling is an env-var change + redeploy; runtime toggling is out of scope (the source behind `isEnabled` can later swap to a cached Firestore `config` read).
