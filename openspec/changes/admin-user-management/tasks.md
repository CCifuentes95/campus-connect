...
- [ ] 1.1 Re-pull `Users.dc.html` and `User Detail.dc.html` via DesignSync; confirm the
...
- [ ] 1.3 Write the ADR for deploying Cloud Functions in this MVP (Context → Decision →
...
- [ ] 2.1 Add `staffType`, `dept`, `cohort`, `studentId`, `cats`, `bookable` to the
...
- [ ] 2.2 Extend the `users/{uid}` update rule in `firestore.rules` — it is a **denylist**, so
      add `unchanged('staffType')`, `unchanged('dept')`, `unchanged('cohort')`,
      `unchanged('studentId')`, `unchanged('cats')`, `unchanged('bookable')` alongside the
      existing `role`/`uid`/`email` clauses
- [ ] 2.3 Update the stale rules comment that says profiles are created by `onUserCreate` —
      they are now created by `adminManageUser`
- [ ] 2.4 Deploy rules: `firebase deploy --only firestore:rules`

## 3. Cloud Function: adminManageUser

- [ ] 3.1 Extract the claim-write + profile-mirror logic from the existing `setRole` callable
      (`functions/src/index.ts:66`) into a shared helper; leave `setRole` exported and working
- [ ] 3.2 Add `adminManageUser` as an `onCall` in `functions/src/index.ts`; reject unless
      `request.auth?.token.role === "admin"` **before any write**, matching `setRole`'s guard
- [ ] 3.3 Implement `action: 'create'` — Auth `createUser`, set the claim (`advisor` for both
...
- [ ] 3.4 Implement `action: 'update'` — update Auth email/password when supplied, update the
...
- [ ] 3.5 Implement `action: 'delete'` — reject when `uid === request.auth.uid` (self-delete
      guard), else delete the Auth account and the `users/{uid}` doc
- [ ] 3.6 zod-validate every payload inside the function before any write; enforce the
...
- [ ] 3.9 Confirm in the Firebase console that `onUserCreate` is not deployed

## 4. Server action

- [ ] 4.1 Add `lib/actions/admin-users.ts` following the `useActionState` discriminated result
      (`idle | error{fieldErrors,values} | success`) pattern from `lib/actions/tickets.ts`
- [ ] 4.2 zod-validate at the boundary (name, email format, password ≥ 8, role in
      Student/Advisor/Staff for create), then invoke `adminManageUser`
- [ ] 4.3 Map callable `HttpsError` codes to `fieldErrors` (duplicate email → email field,
      short password → password field, permission-denied → form-level)
- [ ] 4.4 `revalidatePath("/admin/users")` and the `[id]` path on success

## 5. UI: /admin/users roster

- [ ] 5.1 Create `app/(admin)/admin/users/page.tsx` — RSC read of `users` via
...
- [ ] 5.2 Header ("People & access" + gold "Create user" button) and the subtitle count line
- [ ] 5.3 Four summary tiles — Students, Advisors ("N bookable this term"), Support staff,
      Administrators — with teal-tinted icon squares
- [ ] 5.4 Five role tabs with counts (Everyone/Students/Advisors/Staff/Admins); Advisors and
      Staff filter on `staffType`, not on the claim
- [ ] 5.5 Search field (name, email, programme/department, student ID) with a Clear affordance
- [ ] 5.6 Sortable table: Person (avatar + name/email), Role pill, Program / department
...
- [ ] 5.7 Create-user modal: three role cards (Student/Advisor/Staff), conditional Student
...
- [ ] 5.8 Wire create/delete via `useTransition` calling the server action directly then
...
- [ ] 5.9 Delete-confirm modal reusing the mockup's copy ("This permanently removes the account
...
- [ ] 6.1 Create `app/(admin)/admin/users/[id]/page.tsx` (remember `params` is async) — header
      with avatar, name, role pill, and Delete/Save actions; two-column layout
- [ ] 6.2 Edit form matching the create fields, plus Admin as a selectable role here
- [ ] 6.3 Role-change warning banner (access changes; lands on next token refresh)
- [ ] 6.4 Sidebar: account meta, recent activity / current workload, access summary
- [ ] 6.5 Wire Save/Delete to the server action; hide or disable Delete on the signed-in
      admin's own profile

## 7. Nav

- [ ] 7.1 Re-enable the admin "Users" link in `components/nav/top-nav.tsx` and remove the
      deferral comment
- [ ] 7.2 Confirm the `(admin)` layout guard already covers the new routes (no layout change
      expected)
